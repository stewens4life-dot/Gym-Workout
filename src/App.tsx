...
import { 
  Calendar, Plus, BarChart2, Save, Trash2, Dumbbell, ChevronLeft, ChevronRight, Activity, TrendingUp, Layers, Loader, Cloud, LogIn, User, LogOut
} from 'lucide-react';
...
import { 
  ...
  signInWithPopup,              // Iniciar sesión con Popup
  setPersistence,               // Persistencia de sesión
  browserLocalPersistence,      // Persistencia local
  signOut                       // Cerrar sesión
} from 'firebase/auth';
import { getFirestore, doc, setDoc, onSnapshot, collection, query } from 'firebase/firestore';

// --- CONFIGURACIÓN DE FIREBASE PROPORCIONADA POR EL USUARIO ---
// Usamos la configuración de Firebase Console directamente
...
// Extracción del AppId y ProjectId para rutas de Firestore
...
const App = () => {
  ...
  const [user, setUser] = useState(null);               // Almacena el objeto User de Firebase
  const [userId, setUserId] = useState(null);
  const [isAuthReady, setIsAuthReady] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [showAuthModal, setShowAuthModal] = useState(false); // Muestra el modal de login

  // Variable global para autenticación inicial (puede ser undefined)
  const initialAuthToken = typeof __initial_auth_token !== 'undefined' ? __initial_auth_token : null;

  // --- 1. Inicialización y Autenticación de Firebase (Google Sign-In) ---
  useEffect(() => {
    // 1. Inicializar Firebase con la configuración proporcionada
    ...
    setPersistence(authInstance, browserLocalPersistence)
        .catch((error) => console.error("Error al configurar la persistencia:", error));

    // 3. Intento de autenticación con token de Canvas
    const authenticate = async () => {
        if (initialAuthToken) {
            try {
                // Si existe el token de Canvas, úsalo.
    ...
    // 4. Listener de estado de autenticación (la fuente de verdad)
    const unsubscribe = onAuthStateChanged(authInstance, (user) => {
      if (user) {
        setUser(user);
        setUserId(user.uid);
        setShowAuthModal(false); // Ocultar si hay sesión activa
      } else {
        setUser(null);
        setUserId(null);
        // Si no hay usuario y ya terminó de cargar, mostrar modal
        if (!isLoading) {
  ...
  // --- 1.5. Mostrar Modal si no hay Sesión (Efecto de validación inicial) ---
  useEffect(() => {
      // Si la carga inicial terminó y NO hay usuario, forzar el modal de autenticación
  ...
  const signInWithGoogle = async () => {
    ...
    try {
        ...
        // Si es exitoso, onAuthStateChanged se dispara, establece el usuario y oculta el modal.
    } catch (error) {
  ...
  const handleSignOut = async () => {
    ...
    try {
        await signOut(auth);
        setWorkouts([]); // Limpiar datos del usuario anterior
        setView('dashboard');
        // onAuthStateChanged se dispara, pone userId a null, y el useEffect 1.5 mostrará el modal.
    } catch (error) {
  ...
  useEffect(() => {
    // Si no hay usuario autenticado (UID persistente), detenemos la carga de datos y limpiamos.
    if (!isAuthReady || !db || !userId) {
      setWorkouts([]); 
      return;
    }
    
    // Ruta de la colección: /artifacts/{APP_ID}/users/{userId}/workouts
    // Usamos APP_ID basado en la configuración proporcionada
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        loadedWorkouts.push(doc.data());
      });
      setWorkouts(loadedWorkouts);
      console.log(`Workouts loaded for user ${userId}: ${loadedWorkouts.length}`);
    }, (error) => {
      console.error("Error fetching workouts from Firestore:", error);
    });

    return () => unsubscribe();
  }, [isAuthReady, db, userId, APP_ID]); // Dependencia APP_ID usada en la ruta
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // Solo si está autenticado y hay datos, se colorea el calendario.
      ...
      days.push(
        <button 
          ...
          onClick={() => {
            if (!userId) {
                setShowAuthModal(true); // Bloquear navegación al log si no hay sesión
                return;
            }
  ...
  // --- Lógica de Registro (Logger) (sin cambios) ---
  const handleAddExercise = (exerciseName) => {
    // Si no hay userId, no permitimos agregar ejercicios
    if (!userId) {
        setShowAuthModal(true);
        return;
    }
  ...
  const updateSet = (exerciseIndex, setIndex, field, value) => {
    // Si no hay userId, no permitimos editar sets
    if (!userId) {
        setShowAuthModal(true);
        return;
    }
  ...
  const addSet = (exerciseIndex) => {
    // Si no hay userId, no permitimos agregar sets
    if (!userId) {
        setShowAuthModal(true);
        return;
    }
  ...
  const removeSet = (exerciseIndex, setIndex) => {
    // Si no hay userId, no permitimos remover sets
    if (!userId) {
        setShowAuthModal(true);
        return;
    }
  ...
  // --- 3. Guardar en Firestore (Validación Estricta) ---
  const saveWorkout = async () => {
    // VALIDACIÓN CRÍTICA: Debe haber usuario y DB lista
    if (!db || !userId) {
      console.error('Error: La base de datos no está lista o no hay usuario autenticado.');
      // Muestra el modal en lugar de un alert si no hay userId
      if (!userId) {
        setShowAuthModal(true);
      } else {
        // En caso de que db sea null por alguna razón, pero el usuario esté, mostramos error
    ...
    try {
        // Ruta del documento: /artifacts/{APP_ID}/users/{userId}/workouts/{logDate}
  ...
  useEffect(() => {
    if (view === 'log' && userId) { // Solo cargar si estamos en LOG y hay userId
  ...
  }, [logDate, view, workouts, userId]); // Dependencia userId añadida

  // --- Lógica de Estadísticas (Validación con userId) ---
  const [statExercise, setStatExercise] = useState('Press Banca');
  
  // Datos para Gráfico 1: Ejercicio Individual (Peso Máximo) - Solo se procesan si hay workouts
  ...
  // Datos para Gráfico 2: Promedio General por Grupo Muscular - Solo se procesan si hay workouts
  const overallProgressData = useMemo(() => {
    ...
    sortedWorkouts.forEach(workout => {
      ...
      workout.exercises.forEach(ex => {
        ex.sets.forEach(set => {
          ...
          const r = parseFloat(set.reps) || 0; // Usar reps para volumen, aunque aquí solo promediamos peso
          if (w > 0) {
            totalWeight += w;
            totalSets += 1;
          }
    ...
    // Convertir el mapa a un array de puntos de datos para el gráfico
  ...
  const allExercisesList = useMemo(() => {
    ...
    if (userId) { // Solo añadir ejercicios de workouts si el usuario está logueado
  ...
  // --- Componente Modal de Autenticación con Desenfoque ---
  const AuthModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 rounded-xl p-8 max-w-sm w-full shadow-2xl transform transition-all scale-100 animate-fade-in-up">
        ...
        <button
          ...
          className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-3 transition-all shadow-lg shadow-blue-900/50"
        >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-20 md:pb-0">
      ...
      {/* Contenido principal, se desactiva si el modal está visible */}
      <main className={`max-w-4xl mx-auto p-4 ${showAuthModal ? 'pointer-events-none opacity-50' : ''}`}>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                ...
                <div className="text-3xl font-bold text-white">
                    {/* Solo muestra datos si hay userId */}
            ...
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
              ...
              <div className="space-y-3">
                {userId && workouts.sort((a,b) => new Date(b.date) - new Date(a.date)).slice(0, 3).map(workout => (
                ...
                {(!userId || workouts.length === 0) && <p className="text-slate-500 italic">No hay entrenamientos recientes.</p>}
              </div>
            </div>
        ...
        {view === 'log' && (
          <div className="animate-fade-in pb-10">
            {/* INPUTS DE CONTROL SOLO PARA LECTURA SI NO HAY userId */}
            <div className="flex justify-between items-center mb-6">
              ...
              <input 
                ...
                disabled={!userId} // Deshabilitado si no hay sesión
              />
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {Object.keys(ARNOLD_SPLIT).map(split => (
                  <button
                    key={split}
                    onClick={() => {
                        if (userId) setSelectedSplit(split);
                ...
                <div key={exercise.id} className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button 
                      ...
                      className={`text-red-500 hover:text-red-400 p-1 ${!userId ? 'opacity-50 cursor-not-allowed' : ''}`}
                      disabled={!userId} // Deshabilitado si no hay sesión
                    >
                  ...
                  <div className="space-y-2">
                      ...
                      <div key={setIndex} className="grid grid-cols-6 gap-2 items-center">
                        ...
                        <div className="col-span-1 text-center">
                          <button 
                            ...
                            className={`text-slate-600 hover:text-red-400 ${!userId ? 'opacity-50 cursor-not-allowed' : ''}`}
                            disabled={!userId}
                          >
                            ×
                          </button>
                    ...
                    <button 
                      ...
                      className={`w-full py-2 mt-2 text-xs font-bold text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded border border-dashed border-slate-600 hover:border-slate-500 transition-all ${!userId ? 'opacity-50 cursor-not-allowed' : ''}`}
                      disabled={!userId} // Deshabilitado si no hay sesión
                    >
                      + AGREGAR SERIE
                    </button>
            ...
            <div className={`mt-6 p-4 bg-slate-800/50 rounded-xl border border-dashed border-slate-700 ${!userId ? 'opacity-50 pointer-events-none' : ''}`}>
            ...
            <button 
              onClick={saveWorkout}
              className={`w-full mt-8 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 sticky bottom-24 md:static ${!userId ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={!userId} // Deshabilitado si no hay sesión
            >
            ...
            {!userId && (
                <p className="text-center text-sm text-yellow-500 mt-2">Debes iniciar sesión con Google para guardar tu progreso.</p>
            )}
          </div>
        )}
        ...
        {view === 'stats' && (
          <div className="space-y-6 animate-fade-in pb-10">
            {/* Aviso de datos si no hay sesión */}
            {!userId && (
            ...
            {/* Gráfico 1: Ejercicio Específico */}
             <div className={`bg-slate-900 p-6 rounded-xl border border-slate-800 ${!userId ? 'opacity-50' : ''}`}>
                ...
                <div className="h-64 w-full">
                  {overallProgressData.length > 0 ? (
                    ...
                    <div className="h-full flex items-center justify-center text-slate-500 flex-col">
                      ...
                      <p>{userId ? 'Registra entrenamientos variados para ver la comparativa.' : 'Inicia sesión para cargar datos.'}</p>
                    </div>
                  )}
                </div>
             </div>
      ...
      <nav className={`hidden md:flex fixed left-0 top-20 bottom-0 w-20 flex-col items-center py-8 gap-8 border-r border-slate-800/50 ${showAuthModal ? 'pointer-events-none' : ''}`}>
